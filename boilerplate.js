...
function removeModules (m) {
  var remove = spawn('npm', ['uninstall', m])
}

const installModules = (m)  => {
  let install = spawn('npm', ['install', '--save', m])
  install.stdout.on('data', data => {
    let message = `installing ${m}`
    return message
  })
}
...
function wbp (code, pathToFile) { // I want to get rid of this
...
async function writeBoilerplate (policy) { // need to refactor this
  try {
    let securityFile = `'use strict'

module.exports = {\n`
    var secFolder = './security'
    folderCheck(secFolder)
    if (policy.appDependencies.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/dependencies.js', './security/dependencies.js', err => {
    ...
    if (policy.accessControlsPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/authentication.js', './security/authentication.js', err => {
        if (err) { console.log(err.message) }
      })
      fs.copyFile(__dirname + '/security-default/fire_auth.js', './security/fire_auth.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.secretStorage.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/secrets.js', './security/secrets.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.formProtection.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/forms.js', './security/forms.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.sessionPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/sessions.js', './security/sessions.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.apiPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/api.js', './security/api.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.securityHeaders.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/headers.js', './security/headers.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.securityHeaders.caching.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/cache.js', './security/cache.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.contentValidationPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/validation.js', './security/validation.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.dbSecurityPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/database.js', './security/database.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.connectionPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/connections.js', './security/connections.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.resourceSharingPolicy.corsSettings.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/cors.js', './security/cors.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
    ...
    if (policy.loggingPolicy.enabled === true) {
      ...
      fs.copyFile(__dirname + '/security-default/logging.js', './security/logging.js', err => {
        if (err) { console.log(err.message) }
      })
    } else {
...
module.exports = {
  ...
  fetchModules : (policy) => {
    let writeable = []
    for (let keys in policy) { // for each key in the policy
      // if the policy is 'enabled' add the module to be fetched to an array
      if (keys === 'appDependencies' && policy[keys].enabled === true) { writeable.push('spartan-dependencies') }
      if (keys === 'accessControlsPolicy' && policy[keys].enabled === true) { writeable.push('spartan-access') }
      if (keys === 'secretStorage' && policy[keys].enabled === true) { writeable.push('spartan-secrets') }
      if (keys === 'formProtection' && policy[keys].enabled === true) { writeable.push('spartan-forms') }
      if (keys === 'apiPolicy' && policy[keys].enabled === true) { writeable.push('spartan-apis') }
      if (keys === 'securityHeaders' && policy[keys].enabled === true) { writeable.push('spartan-headers') }
      if (keys === 'contentValidationPolicy' && policy[keys].enabled === true) { writeable.push('spartan-validation') }
      if (keys === 'dbSecurityPolicy' && policy[keys].enabled === true) { writeable.push('spartan-database') }
      if (keys === 'connectionPolicy' && policy[keys].enabled === true) { writeable.push('spartan-connections') }
      if (keys === 'resourceSharingPolicy' && policy[keys].corsSettings.enabled === true) { writeable.push('spartan-cors') }
      if (keys === 'loggingPolicy' && policy[keys].enabled === true) { writeable.push('spartan-logging') }
    }
    // fetch every module in the array from npm and write the values to security.js
    for (let a = 0; a < writeable.length; a++) {
      installModules(writeable[a])
    }
  },
  writeBoilerplate : writeBoilerplate,
